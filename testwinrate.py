...
# =========================
# Pair 采样（保留原模式；新增 by_delta_all）
# =========================
def sample_pairs(utts, meta, mode="by_delta_all", max_pairs=None, seed=1234, delta=0.03):
    """
    新增模式 by_delta_all（默认）：
      - 先按 |label_i - label_j| > delta 过滤
      - 再（如给定 max_pairs）做均匀随机抽样，否则全量保留

    其他模式（within_spk / cross_spk / all_pairs）保持原有逻辑不变。
    """
    rng = random.Random(seed)

    # ===== 新模式：按差值阈值全量匹配 =====
    if mode == "by_delta_all":
        labels = [float(meta[u]["label"]) for u in utts]
        N = len(utts)
        pairs = []
        for i in range(N):
            li = labels[i]
            for j in range(i + 1, N):
                if abs(li - labels[j]) > delta:
                    pairs.append((i, j))
        # 均匀抽样（若设定了 max_pairs）
...
def evaluate_winrate(utts, mu, var, meta,
                     pairs,
                     epsilon_label=0.0,          # 改：默认 0，避免二次过滤
                     tie_tol=1e-6,
    ...
    for i, j in pairs:
        ...
        # 这里保留逻辑，但默认 epsilon_label=0，不会再过滤
...
def main(args):
    ...
    # ====== win-rate 流程：使用“按差值全量匹配” ======
    pairs = sample_pairs(
        utts=utts,
        meta=meta,
        mode=args.pair_mode,              # 默认 by_delta_all
        max_pairs=args.max_pairs,         # 若给定则均匀抽样到该上限
        seed=args.seed,
        delta=args.epsilon_label          # 用同一个阈值在采样阶段筛选
    )
    # 评测阶段避免二次过滤：将 epsilon_label 置 0
    ...
    report = {
        "settings": {
            ...
            "epsilon_label": args.epsilon_label,   # 记录用于“采样筛选”的阈值
            "tie_tol": args.tie_tol,
            "use_uncertainty": args.use_uncertainty,
            "z": args.z,
...
if __name__ == "__main__":
    ...
    # 评测类型
    ap.add_argument("--eval", type=str, default="winrate", choices=["mse", "winrate"],
                    help="选择评测方式：mse（逐样本回归）或 winrate（成对方向一致率）")

    # 配对模式：默认 by_delta_all（任意满足阈值的都会匹配）
    ap.add_argument("--pair-mode", type=str, default="by_delta_all",
                    choices=["by_delta_all", "within_spk", "cross_spk", "all_pairs"],
                    help="by_delta_all：先按差值阈值筛选，再全量配对；其他模式保留兼容。")

    # 差值阈值：用于“采样阶段筛选”，评测阶段会固定用 0 以避免二次过滤
    ap.add_argument("--epsilon-label", type=float, default=0.03,
                    help="仅在采样阶段保留 |label_i - label_j| > 该阈值 的样本对")
    ...
    # 最大测试样本对数：若设定，则在“全量满足阈值的对”中等概率抽样到该上限；不设定则全量评测
